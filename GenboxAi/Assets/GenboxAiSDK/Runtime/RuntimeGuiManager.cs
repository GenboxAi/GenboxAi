...
namespace GenboxAiSDK
{
    public class RuntimeGuiManager : MonoBehaviour
    {
        ...
        private SkyboxGenerator _generator;
        public SkyboxGenerator Generator
        {
        ...
        [SerializeField]
        private SkyboxMesh _skybox;
        public SkyboxMesh Skybox
        {
            get { return _skybox; }
            set { _skybox = value; }
        }
        ...
        {
            if (_generator.CurrentState == SkyboxGenerator.State.Ready)
            {
            ...
            {
                selectable.interactable = _generator.CurrentState == SkyboxGenerator.State.Ready;
            }

            var disabledColors = _promptPanel.GetComponentsInChildren<DisabledColor>();
            foreach (var disabledColor in disabledColors)
            {
                disabledColor.Disabled = _generator.CurrentState != SkyboxGenerator.State.Ready;
            }
        ...
        private void UpdateCanRemix()
        {
            ...
            _remixButton.interactable = _generator.CurrentState == SkyboxGenerator.State.Ready && canRemix;
        ...
        {
            _meshCreatorButton.interactable = _skybox.HasDepthTexture && _generator.CurrentState == SkyboxGenerator.State.Ready;
        ...
        {
            if (_generator.CurrentState == SkyboxGenerator.State.Generating)
            {
                _generator.Cancel();
                return;
            }
        ...
        private void UpdateCamera()
        {
            ...
            {
                _demoCamera.SetMode(DemoCamera.Mode.MeshCreator);
            }
            else if (_showSpheresToggle.IsOn)
            {
                _demoCamera.SetMode(DemoCamera.Mode.CenterOrbit);
            }
            else
            {
                _demoCamera.SetMode(DemoCamera.Mode.SkyboxDefault);
            }
        }
        ...
        {
            _generateButton.interactable = _generator.CurrentState != SkyboxGenerator.State.NeedApiKey;
